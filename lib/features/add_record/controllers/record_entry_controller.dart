...
class RecordEntryController extends ChangeNotifier {
  ...
  List<ImprovementOption> get improvementOptions => _visibleImprovementOptions();
  ...
  bool get canAdjustPoints => selectedContent?.allowAdjust ?? true;

  int get minPoints => selectedContent?.minPoints ?? 1;

  int get maxPoints => selectedContent?.maxPoints ?? 4;

  bool get isLineTestCategory => selectedCategory?.name == '行测';

  bool get isEssayCategory => selectedCategory?.name == '申论';

  bool get isEnglishCategory => selectedCategory?.name == '英语';

  List<String> get outputTypeOptions => const ['练字', '摘抄', '概括', '对策', '公文', '作文'];
  ...
  Future<void> load({bool preserveSelection = false}) async {
    try {
      ...
      final weaknesses = await optionsRepository.getWeaknessOptions(includeDisabled: false);
      final improvements = await optionsRepository.getImprovementOptions(includeDisabled: false);
      ...
      _weaknessOptions = weaknesses;
      _improvementOptions = improvements;
  ...
  void setPoints(int value) {
    if (!canAdjustPoints) {
      points = selectedContent?.defaultPoints ?? value;
    } else {
      points = value.clamp(minPoints, maxPoints);
    }
    notifyListeners();
  }
  ...
  void toggleWeaknessTag(String tag) {
    if (weaknessTags.contains(tag)) {
      weaknessTags = weaknessTags.where((item) => item != tag).toList(growable: false);
    } else {
  ...
  Future<void> save({required bool continueAdding}) async {
    ...
    try {
      ...
      final record = StudyRecord(
        ...
        feedbackOptionId: selectedReward?.id,
        feedbackNameSnapshot: selectedReward!.name,
  ...
  void _applyDefaultSelection() {
    ...
    pomodoroCount = 1;
    points = selectedContent?.defaultPoints ?? 1;
    _resetDetailFields();
    _autoTrackCurrentTime = true;
    occurredAt = DateTime.now();
  }
  ...
  void _reconcileSelections() {
    ...
    if (selectedContent != null) {
      final matched = _contentOptions.where((item) => _sameContent(item, selectedContent!));
      if (matched.isNotEmpty) {
    ...
    if (selectedReward != null) {
      final matched = _rewardOptions.where((item) => _sameReward(item, selectedReward!));
      if (matched.isNotEmpty) {
  ...
  List<WeaknessOption> _visibleWeaknessOptions() {
    final selectedCategoryId = selectedCategory?.id;
    return _weaknessOptions.where((item) {
  ...
  List<ImprovementOption> _visibleImprovementOptions() {
    final selectedCategoryId = selectedCategory?.id;
    return _improvementOptions.where((item) {
      return item.isEnabled &&
          (item.categoryId == null ||
              (selectedCategoryId != null && item.categoryId == selectedCategoryId));
    }).toList(growable: false);
  ...
  void _applySelectedContentDefaults({required bool resetDetailFields}) {
    ...
    if (content != null) {
      points = content.defaultPoints;
    }
    if (resetDetailFields) {
      _resetDetailFields();
      if (isEssayCategory) {
        outputType = _defaultOutputTypeForSelectedContent();
      }
    }
  }
  ...
  String? _defaultOutputTypeForSelectedContent() {
