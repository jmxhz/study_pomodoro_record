...
import 'manage_contents_page.dart';
      ...
      builder: (context, controller, child) {
        return Scaffold(
          ...
          body: controller.isLoading
              ? const LoadingView(message: '正在加载设置...')
              : controller.errorMessage != null &&
                      controller.categories.isEmpty &&
                      controller.contentOptions.isEmpty &&
                      controller.rewardOptions.isEmpty
                  ? Padding(
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const Text(
                                  '导出会生成 categories.csv、content_options.csv、reward_options.csv、study_records.csv 四个文件，并支持直接分享。',
                                ),
                                const SizedBox(height: 16),
                                FilledButton.icon(
  ...
  Future<void> _importCsv(
    ...
    final mode = await showDialog<CsvImportMode>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text('追加导入会保留现有数据；覆盖导入只会覆盖本次选择的 CSV 类型。'),
          actions: [
            TextButton(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '导入成功：分类 ${summary.categories}，内容 ${summary.contentOptions}，奖励 ${summary.rewardOptions}，记录 ${summary.studyRecords}',
          ),
        ),
      );
    } catch (error) {
