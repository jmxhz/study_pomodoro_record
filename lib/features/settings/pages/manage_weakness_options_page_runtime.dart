...
class ManageWeaknessOptionsPage extends StatelessWidget {
  const ManageWeaknessOptionsPage({super.key});
...
class _WeaknessList extends StatelessWidget {
  ...
  final ValueChanged<WeaknessOption> onEdit;
  final ValueChanged<WeaknessOption> onDelete;

  @override
  Widget build(BuildContext context) {
    final sections = _buildSections(controller);
    if (sections.isEmpty) {
      return const Center(child: Text('暂无薄弱点选项'));
    }
                ...
                children: [
                  ...
                  ReorderableListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Container(
                        key: ValueKey(
                            'weakness-${section.groupKey}-${item.id ?? item.name}'),
  ...
  List<_WeaknessSection> _buildSections(SettingsController controller) {
    final contentGroups = <int?, List<WeaknessOption>>{};
    final legacyCategoryGroups = <int?, List<WeaknessOption>>{};
    for (final item in controller.weaknessOptions) {
      if (item.contentOptionId != null || item.categoryId == null) {
        contentGroups
            .putIfAbsent(item.contentOptionId, () => <WeaknessOption>[])
            .add(item);
      } else {
    ...
    final sections = <_WeaknessSection>[
      ...[
        ...
        if (contentGroups.containsKey(null)) null,
          ...
          .map(
            (contentOptionId) => _WeaknessSection(
